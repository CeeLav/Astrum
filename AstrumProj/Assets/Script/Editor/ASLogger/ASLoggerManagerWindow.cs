...
namespace Astrum.Editor.ASLogger
{
    ...
    public class ASLoggerManagerWindow : EditorWindow
    {
        ...
        private void OnEnable()
        {
            ...
            {
                _config = Astrum.CommonBase.ASLogger.CurrentConfig;
                _isInitialized = true;
                UpdateStatistics();
                return;
            }
        ...
        private void LoadConfig()
        {
            ...
            {
                // 尝试从项目根目录加载
                var configs = Resources.FindObjectsOfTypeAll<LogManagerConfig>();
                if (configs.Length > 0)
                {
                    _config = configs[0];
                }
                else
                {
                    ...
                    _config = CreateInstance<LogManagerConfig>();
            ...
            if (_config != null)
            {
                ...
                // 通知ASLogger加载配置
                Astrum.CommonBase.ASLogger.SetConfig(_config);
        ...
        private void RefreshData()
        {
            try
            {
                ...
                // 扫描项目中的实际日志
                var projectLogs = LogScanner.ScanProject(_config);
