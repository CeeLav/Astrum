...
namespace Astrum.LogicCore.Capabilities
{
	...
	/// </summary>
	[MemoryPackable]
	public partial class SkillCapabilityOld : Capability
	{
		public override void Initialize()
		{
			...
			{
				ASLogger.Instance.Error($"SkillCapabilityOld.Initialize: SkillComponent not found on entity {OwnerEntity?.UniqueId}");
				return;
			}
			...
			{
				ASLogger.Instance.Error($"SkillCapabilityOld.Initialize: ActionComponent not found on entity {OwnerEntity?.UniqueId}");
				return;
			}
		...
		public bool LearnSkill(int skillId, int level = 1)
		{
			...
			if (skillComponent == null)
			{
				ASLogger.Instance.Error("SkillCapabilityOld.LearnSkill: SkillComponent not found");
				return false;
			}

			if (skillComponent.HasSkill(skillId))
			{
				ASLogger.Instance.Warning($"SkillCapabilityOld.LearnSkill: Skill {skillId} already learned");
				return false;
			}
			...
			if (skillInfo == null)
			{
				ASLogger.Instance.Error($"SkillCapabilityOld.LearnSkill: Skill {skillId} not found in config");
				return false;
			}
			...
			ASLogger.Instance.Info($"SkillCapabilityOld.LearnSkill: Entity {OwnerEntity?.UniqueId} learned skill {skillId} (Lv{level})");
			return true;
		}
		...
		public bool ForgetSkill(int skillId)
		{
			...
			if (removed)
			{
				ASLogger.Instance.Info($"SkillCapabilityOld.ForgetSkill: Entity {OwnerEntity?.UniqueId} forgot skill {skillId}");
			}
			return removed;
		}
		...
		private void RegisterSkillActions(SkillInfo skill)
		{
			...
			foreach (var actionId in skill.SkillActionIds)
			{
				...
				if (!actionComponent.AvailableActions.ContainsKey(action.Id))
				{
					...
					ASLogger.Instance.Debug($"SkillCapabilityOld: Registered action {action.Id} for skill");
				}
			}
		}
...
/*
			foreach (var action in skill.SkillActions)
			{
				...
				ASLogger.Instance.Debug($"SkillCapabilityOld: Unregistered action {action.Id} for skill");
			}*/
		}
		...
		public override bool CanExecute()
		{
			...
			return OwnerHasComponent<SkillComponent>() && OwnerHasComponent<ActionComponent>();
		}
	}
}
