...
namespace Astrum.LogicCore.Capabilities
{
    ...
    public partial class DeadCapabilityOld : Capability
    {
        ...
        private void OnEntityDied(EntityDiedEventData eventData)
        {
            ...
            if (_isDead)
            {
                ASLogger.Instance.Warning($"[DeadCapabilityOld] Entity {OwnerEntity.UniqueId} already dead, ignoring duplicate event");
                return;
            }
            
            _isDead = true;
            ASLogger.Instance.Info($"[DeadCapabilityOld] Entity {OwnerEntity.UniqueId} died, disabling non-whitelisted capabilities");
        ...
        private void OnEntityRevived(EntityRevivedEventData eventData)
        {
            ...
            if (!_isDead)
            {
                ASLogger.Instance.Warning($"[DeadCapabilityOld] Entity {OwnerEntity.UniqueId} not dead, ignoring revive event");
                return;
            }
            
            _isDead = false;
            ASLogger.Instance.Info($"[DeadCapabilityOld] Entity {OwnerEntity.UniqueId} revived, restoring capabilities");
        ...
        private void DisableNonWhitelistedCapabilities()
        {
            ...
            foreach (var capability in OwnerEntity.Capabilities)
            {
                ...
                if (capability.IsActive)
                {
                    ...
                    ASLogger.Instance.Debug($"[DeadCapabilityOld] Disabled capability: {capability.Name}");
                }
            }
        }
        ...
        private void RestoreDisabledCapabilities()
        {
            foreach (var capability in _disabledCapabilities)
            {
                ...
                ASLogger.Instance.Debug($"[DeadCapabilityOld] Restored capability: {capability.Name}");
            }
