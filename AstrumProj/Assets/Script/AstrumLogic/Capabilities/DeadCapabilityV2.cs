...
namespace Astrum.LogicCore.Capabilities
{
    ...
    /// 集中管理实体死亡后的能力停用/恢复
    /// </summary>
    public class DeadCapabilityV2 : Capability<DeadCapabilityV2>
    {
        // ====== 元数据 ======
        ...
        private void OnEntityDied(Entity entity, EntityDiedEventData eventData)
        {
            ...
            if (isDead)
            {
                ASLogger.Instance.Warning($"[DeadCapabilityV2] Entity {entity.UniqueId} already dead, ignoring duplicate event");
                return;
            }
            ...
            ASLogger.Instance.Info($"[DeadCapabilityV2] Entity {entity.UniqueId} died, disabling non-whitelisted capabilities");
        ...
        private void OnEntityRevived(Entity entity, EntityRevivedEventData eventData)
        {
            ...
            if (!isDead)
            {
                ASLogger.Instance.Warning($"[DeadCapabilityV2] Entity {entity.UniqueId} not dead, ignoring revive event");
                return;
            }
            ...
            ASLogger.Instance.Info($"[DeadCapabilityV2] Entity {entity.UniqueId} revived, restoring capabilities");
        ...
        private void DisableNonWhitelistedCapabilities(Entity entity)
        {
            ...
            foreach (var kvp in entity.CapabilityStates)
            {
                ...
                ASLogger.Instance.Debug($"[DeadCapabilityV2] Disabled new capability TypeId: {typeId}");
            }
            
            // 处理旧系统：遍历旧架构的 Capability 实例
            // 注意：旧系统使用 Type 作为白名单，需要转换为 TypeId
            var whitelistTypes = new HashSet<Type> { typeof(DeadCapabilityV2) };
            ...
            foreach (var oldCapability in entity.Capabilities)
            {
                ...
                if (oldCapability.IsActive)
                {
                    ...
                    ASLogger.Instance.Debug($"[DeadCapabilityV2] Disabled old capability: {oldCapability.Name}");
                }
            }
        ...
        private void RestoreDisabledCapabilities(Entity entity)
        {
            ...
            if (disabledTypeIds != null && disabledTypeIds.Count > 0)
            {
                foreach (var typeId in disabledTypeIds)
                {
                    ...
                    ASLogger.Instance.Debug($"[DeadCapabilityV2] Restored new capability TypeId: {typeId}");
                }
            }
            ...
            foreach (var oldCapability in entity.Capabilities)
            {
                ...
                // 跳过白名单类型
                if (oldCapability.GetType() == typeof(DeadCapability) || 
                    oldCapability.GetType() == typeof(DeadCapabilityV2))
                    continue;
                ...
                if (!oldCapability.IsActive)
                {
                    ...
                    ASLogger.Instance.Debug($"[DeadCapabilityV2] Restored old capability: {oldCapability.Name}");
                }
            }
