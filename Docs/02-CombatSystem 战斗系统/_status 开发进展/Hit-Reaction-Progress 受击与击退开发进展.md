...
| 版本 | 日期 | 变更内容 | 作者 |
|------|------|----------|------|
| v1.2 | 2025-11-06 | 架构修订：职责分离优化，Handler 计算 + Capability 执行 | AI Assistant |
...
- [受击与击退设计](../技能效果/Hit-Reaction-And-Knockback%20受击与击退.md) - 技术方案（已更新至 v1.2）
...
**问题**：v1.1 设计中，`HitReactionCapability` 直接处理效果逻辑（伤害、击退），并修改组件数据，职责过重且破坏封装性。
...
```
SkillExecutorCapability
  ↓ 调用 SkillEffectSystem.QueueSkillEffect()
SkillEffectSystem.Update()
  ↓ 分发到 EffectHandler
EffectHandler（只读外部，发送事件）
  ├─ DamageEffectHandler
  │   ├─ 读取 Stats（计算伤害）
  │   ├─ 修改 DynamicStats（扣血）
  │   └─ 发送 HitReactionEvent
  ...
  └─ KnockbackEffectHandler
      ├─ 读取 Trans（计算方向）
      ├─ ❌ 不修改 KnockbackComponent
      └─ 发送 KnockbackEvent + HitReactionEvent
...
#### 新增事件：
1. **`KnockbackEvent`** - 击退事件
   - 包含：方向、距离、持续时间、类型
   - 发送方：`KnockbackEffectHandler`
   - 接收方：`KnockbackCapability`

2. **`HitReactionEvent`** - 受击反馈事件
   - 包含：受击方向、效果类型、是否硬直
   - 发送方：所有 EffectHandler
   - 接收方：`HitReactionCapability`

#### 新增文件：
- `Events/KnockbackEvent.cs`
- `Events/HitReactionEvent.cs`
- `SkillSystem/EffectHandlers/KnockbackEffectHandler.cs`

#### 修改文件：
- `SkillExecutorCapability.cs` - 改为调用 SkillEffectSystem
- `HitReactionCapability.cs` - 改为只处理表现，接收 HitReactionEvent
- `KnockbackCapability.cs` - 新增事件处理，接收 KnockbackEvent
- `DamageEffectHandler.cs` - 新增发送 HitReactionEvent
- `SkillEffectSystem.cs` - 注册 KnockbackEffectHandler

#### 删除文件：
- `Events/SkillEffectEvent.cs` - 不再需要（改用 SkillEffectData 队列）

### ⚠️ **设计原则**

#### Handler vs Capability 职责划分：

| 效果类型 | Handler 职责                     | Capability 职责           | 原因                   |
| -------- | -------------------------------- | ------------------------- | ---------------------- |
| **伤害** | 计算 + 扣血 + 发送事件           | 播放受击表现              | 伤害是立即生效的原子操作 |
| **击退** | 计算方向/距离 + 发送事件         | 写入组件 + 应用位移       | 击退是持续过程         |
| **治疗** | 计算 + 加血 + 发送事件（可选）   | 播放治疗表现（可选）      | 治疗是立即生效的       |
| **Buff** | 计算参数 + 发送事件              | 写入 Buff 组件 + 每帧应用 | Buff 是持续过程        |

**核心原则**：
- ✅ **立即生效**的效果 → Handler 直接处理（伤害、治疗）
- ✅ **持续过程**的效果 → Handler 计算 + Capability 执行（击退、Buff）
- ✅ **表现分离**：所有效果都通过 HitReactionEvent 触发表现
...
**确认后需实现**：
- [ ] 创建 `KnockbackEvent`
- [ ] 修改 `KnockbackEffectHandler`（不修改组件，只发事件）
- [ ] 修改 `KnockbackCapability`（接收事件，写入组件）
- [ ] 修改 `DamageEffectHandler`（保持现状，伤害可以直接修改）
- [ ] 测试完整链路
