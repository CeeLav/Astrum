...
- **视觉层平滑展示**：在30-60 FPS的渲染帧中，通过插值和平滑算法，使角色动画连续自然
- **避免误差累积**：通过回归机制和误差钳制，防止长期浮点漂移
...
protected override void OnUpdate(float deltaTime)
{
    ...
    // 2️⃣ 获取动画根骨骼本帧位移（关闭RootMotion后仍可用）
    ...
    if (logicDelta.sqrMagnitude > 0.0001f)
    {
        // 避免视觉突变：逻辑跳变时清空偏移
    ...
    // 5️⃣ 平滑回归逻辑点（防止误差累积）
    _visualSync.visualOffset = Vector3.Lerp(
        _visualSync.visualOffset, 
        Vector3.zero, 
        followSmooth * deltaTime
    );
    
    // 6️⃣ 误差钳制防护（避免浮点长时间漂移）
...
1. **每帧 Lerp 回归**：
   ```csharp
   visualOffset = Vector3.Lerp(visualOffset, Vector3.zero, followSmooth * deltaTime);
   ```
...
**效果**：从根本上避免长期浮点漂移，确保视觉位置始终围绕逻辑位置。
...
namespace Astrum.View.Components
{
    ...
    public class TransViewComponent : ViewComponent
    {
        ...
        [SerializeField] private float followSmooth = 2.0f;      // 视觉回归速度
...
在组件初始化时，需要设置 `lastLogicPos` 的初始值：
...
protected override void OnInitialize()
{
    ...
    {
        var fixedPos = transComponent.Position;
        _visualSync.lastLogicPos = new Vector3(
            (float)fixedPos.x,
            (float)fixedPos.y,
            (float)fixedPos.z
        );
