...
- **逻辑层**：`TransComponent`（定点位置）、`SkillDisplacementCapability`（技能位移）
...
│  SkillDisplacementCapability (技能位移曲线)              │
└─────────────────────────────────────────────────────────┘
                         │ 每逻辑帧更新
                         ↓
┌─────────────────────────────────────────────────────────┐
...
- 在 `TransViewComponent` 中作为私有字段
- 不需要序列化，仅在运行时使用

**逻辑帧间隔常量**：

- 使用项目全局常量：`LSConstValue.UpdateInterval`（定义在 `Astrum.LogicCore.FrameSync.LSConstValue`）
- 当前值：`50` 毫秒（对应 20 FPS）
- 在代码中使用时需转换为秒：`LSConstValue.UpdateInterval / 1000.0f = 0.05f`

### 3.2 组件扩展方案

#### 方案A：扩展 TransViewComponent（✅ 采用）
...
#### 方案B：新建 VisualFollowerComponent（备选）

创建独立的 `VisualFollowerComponent`，专门处理视觉跟随逻辑，职责更清晰。

**本项目采用方案A**，因为视觉跟随与移动表现紧密相关。
...
```csharp
// SkillDisplacementCapability 在每逻辑帧的 Tick() 中：
// 1. 读取 ActionComponent.CurrentAction（技能动作信息）
// 2. 检查是否有 RootMotionData
// 3. 应用当前帧的增量位移和旋转
// 4. 更新 TransComponent.Position（定点数）

// 具体实现参考：SkillDisplacementCapability.ApplyRootMotion()
// - 从 SkillActionInfo.RootMotionData 读取当前帧数据
// - 局部空间转世界空间（基于角色朝向）
...
protected override void OnUpdate(float deltaTime)
{
    ...
    if (logicUpdated)
    {
        ...
        // 使用游戏的逻辑帧间隔常量（LSConstValue.UpdateInterval = 50ms）
...
// 注意：逻辑帧间隔使用游戏全局常量 LSConstValue.UpdateInterval（50ms）
// 在代码中使用：float logicFrameInterval = LSConstValue.UpdateInterval / 1000.0f;
```
...
**逻辑帧间隔**：
- 使用全局常量 `LSConstValue.UpdateInterval`（定义在 `Astrum.LogicCore.FrameSync`）
- 当前值：50ms（20 FPS）
- 在代码中转换为秒：`LSConstValue.UpdateInterval / 1000.0f = 0.05f`

### 4.3 必要的命名空间引用

在 `TransViewComponent.cs` 文件顶部添加必要的命名空间：
...
using Astrum.LogicCore.FrameSync;  // ⚠️ 必须引用此命名空间才能使用 LSConstValue
```

**重要提示**：
- `LSConstValue` 定义在 `Astrum.LogicCore.FrameSync` 命名空间中
- 必须添加 `using Astrum.LogicCore.FrameSync;` 才能使用 `LSConstValue.UpdateInterval`

### 4.4 AnimationViewComponent 集成
...
- **技能位移**：逻辑层通过 `SkillDisplacementCapability` 应用技能位移曲线
...
**更新日志**：
- v1.3 (2025-01-16)：
  - 更新逻辑层组件引用：从 `SkillExecutorCapability` 改为 `SkillDisplacementCapability`
  - 更新架构图和集成说明，反映新的技能位移能力
  - 添加 `SkillDisplacementCapability` 相关文档链接
- v1.2 (2025-01-16)：
  - 明确采用方案A（扩展 TransViewComponent）
  - 使用项目全局常量 `LSConstValue.UpdateInterval` 替代硬编码的逻辑帧间隔
  - 添加必要的命名空间引用 `Astrum.LogicCore.FrameSync`
- v1.1 (2025-01-16)：优化逻辑跳变处理，使用逻辑帧间插值替代直接清空和平滑回归，消除渲染帧与逻辑帧时间间隔的影响
- v1.0 (2025-01-16)：初始版本，基于平滑回归的视觉跟随方案

## 相关文档

- [SkillDisplacementCapability 技能位移能力](./SkillDisplacementCapability%20技能位移能力.md) - 技能位移能力的完整实现方案
