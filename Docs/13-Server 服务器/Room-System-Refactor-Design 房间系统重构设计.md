# 房间系统重构设计

> 📖 **版本**: v1.0 | **最后更新**: 2025-01-27  
> 🎯 **适用范围**: 服务器端房间系统与帧同步架构重构  
> 👥 **面向读者**: 服务器开发人员  
> ✅ **目标**: 完成房间系统面向对象重构，实现 RoomManager 管理房间实例，每个房间独立管理帧同步与战斗逻辑

**TL;DR**
- RoomManager 管理 Room 实例（而非 RoomInfo）
- 每个 Room 实例独立管理自己的帧同步逻辑和战斗逻辑
- FrameSyncManager 简化为房间帧同步的协调器
- 使用面向对象方式组织代码，提高可维护性和扩展性

## 1. 概述

当前服务器架构中，房间管理和帧同步逻辑耦合严重：
- `RoomManager` 只管理 `RoomInfo` 数据，不管理房间实例
- `FrameSyncManager` 承担了所有房间的帧同步逻辑，职责过重
- 房间的帧同步状态（`RoomFrameSyncState`）与房间信息分离
- 战斗逻辑分散在多个地方，难以维护

重构目标：
- **职责分离**：RoomManager 管理房间实例，每个房间管理自己的逻辑
- **面向对象**：使用 Room 类封装房间的所有状态和行为
- **可扩展性**：便于后续添加房间类型、游戏模式等功能

**设计理念**:
- 单一职责：每个类只负责一个明确的职责
- 封装：房间相关的所有逻辑封装在 Room 类中
- 依赖注入：Room 通过构造函数接收必要的依赖

**系统边界**:
- ✅ 负责：房间生命周期管理、帧同步逻辑、战斗逻辑
- ❌ 不负责：网络消息处理、用户管理、匹配系统

## 2. 架构设计

### 2.1 整体架构

```
┌─────────────────────────────────────────────────────────┐
│                    GameServer                            │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  │
│  │ RoomManager  │  │ UserManager  │  │ NetworkMgr   │  │
│  └──────┬───────┘  └──────────────┘  └──────────────┘  │
│         │                                                │
│         │ 管理 Room 实例                                  │
│         ▼                                                │
│  ┌──────────────────────────────────────────────────┐  │
│  │              Room (房间实例)                      │  │
│  │  ┌────────────────────────────────────────────┐  │  │
│  │  │  RoomInfo (房间信息)                       │  │  │
│  │  │  - Id, Name, Status                        │  │  │
│  │  │  - Players, MaxPlayers                      │  │  │
│  │  └────────────────────────────────────────────┘  │  │
│  │  ┌────────────────────────────────────────────┐  │  │
│  │  │  FrameSyncController (帧同步控制器)         │  │  │
│  │  │  - AuthorityFrame                          │  │  │
│  │  │  - InputBuffer                             │  │  │
│  │  │  - ProcessFrame()                          │  │  │
│  │  └────────────────────────────────────────────┘  │  │
│  │  ┌────────────────────────────────────────────┐  │  │
│  │  │  LogicRoom (逻辑房间)                      │  │  │
│  │  │  - World, LSController                     │  │  │
│  │  │  - FrameTick()                             │  │  │
│  │  └────────────────────────────────────────────┘  │  │
│  └──────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────┘
```

### 2.2 核心类设计

#### RoomManager（房间管理器）

**职责**: 管理所有房间实例的生命周期

**核心功能**:
- 创建房间实例
- 查找房间实例
- 销毁房间实例
- 获取房间列表

**设计要点**:
- 使用 `ConcurrentDictionary<string, Room>` 存储房间实例
- 房间ID作为唯一标识
- 提供线程安全的房间操作接口

```csharp
public class RoomManager
{
    private readonly ConcurrentDictionary<string, Room> _rooms = new();
    
    // 创建房间
    public Room CreateRoom(string creatorId, string roomName, int maxPlayers);
    
    // 获取房间
    public Room? GetRoom(string roomId);
    
    // 销毁房间
    public bool DestroyRoom(string roomId);
    
    // 获取所有房间
    public IEnumerable<Room> GetAllRooms();
}
```

#### Room（房间实例）

**职责**: 封装房间的所有状态和行为

**核心功能**:
- 管理房间信息（RoomInfo）
- 管理玩家列表
- 管理帧同步逻辑
- 管理战斗逻辑（LogicRoom）
- 处理房间状态转换

**设计要点**:
- 封装房间的所有状态
- 提供房间操作的统一接口
- 内部管理帧同步和战斗逻辑

```csharp
public class Room
{
    // 房间信息
    public RoomInfo Info { get; private set; }
    
    // 帧同步控制器
    private FrameSyncController _frameSyncController;
    
    // 逻辑房间
    private Room? _logicRoom;
    
    // 玩家列表
    private readonly List<string> _playerIds = new();
    
    // 依赖注入
    private readonly ServerNetworkManager _networkManager;
    private readonly UserManager _userManager;
    
    // 构造函数
    public Room(
        RoomInfo roomInfo,
        ServerNetworkManager networkManager,
        UserManager userManager)
    {
        Info = roomInfo;
        _networkManager = networkManager;
        _userManager = userManager;
        _frameSyncController = new FrameSyncController(this);
    }
    
    // 玩家管理
    public bool AddPlayer(string userId);
    public bool RemovePlayer(string userId);
    
    // 游戏控制
    public void StartGame();
    public void EndGame(string reason);
    
    // 帧同步
    public void Update(); // 由 RoomManager 统一调用
    public void HandleInput(string userId, SingleInput input);
    
    // 状态查询
    public bool IsActive { get; }
    public bool IsPlaying { get; }
}
```

#### FrameSyncController（帧同步控制器）

**职责**: 管理单个房间的帧同步逻辑

**核心功能**:
- 管理帧号（AuthorityFrame）
- 收集和缓存输入数据
- 推进帧逻辑
- 发送帧同步数据

**设计要点**:
- 从 FrameSyncManager 中提取出来，作为 Room 的内部组件
- 封装帧同步相关的所有逻辑
- 通过 Room 访问网络和用户管理

```csharp
public class FrameSyncController
{
    private readonly Room _room;
    
    // 帧同步状态
    public int AuthorityFrame { get; private set; }
    public long StartTime { get; private set; }
    public bool IsActive { get; private set; }
    
    // 输入缓冲区
    private readonly Dictionary<int, Dictionary<long, LSInput>> _frameInputs = new();
    
    // 玩家ID映射
    private readonly Dictionary<string, long> _userIdToPlayerId = new();
    
    public FrameSyncController(Room room)
    {
        _room = room;
    }
    
    // 开始帧同步
    public void Start();
    
    // 停止帧同步
    public void Stop(string reason);
    
    // 更新帧（由 Room.Update 调用）
    public void Update();
    
    // 处理输入
    public void HandleInput(string userId, SingleInput input);
    
    // 处理单帧
    private void ProcessFrame();
}
```

### 2.3 数据流

#### 房间创建流程

```
GameServer.HandleCreateRoomRequest()
    ↓
RoomManager.CreateRoom()
    ↓
new Room(roomInfo, networkManager, userManager)
    ↓
Room 初始化完成，返回给 GameServer
```

#### 帧同步流程

```
GameServer.Update()
    ↓
RoomManager.UpdateAllRooms()
    ↓
foreach Room in _rooms:
    Room.Update()
        ↓
    FrameSyncController.Update()
        ↓
    ProcessFrame()
        ↓
    LogicRoom.FrameTick()
        ↓
    SendFrameSyncData()
```

#### 输入处理流程

```
GameServer.HandleSingleInput()
    ↓
RoomManager.GetRoom(roomId)
    ↓
Room.HandleInput(userId, input)
    ↓
FrameSyncController.HandleInput(userId, input)
    ↓
存储到输入缓冲区
```

## 3. 实现细节

### 3.1 RoomManager 重构

**当前实现**:
- 管理 `RoomInfo` 数据
- 提供房间信息查询接口

**重构后**:
- 管理 `Room` 实例
- 提供房间实例的创建、查找、销毁接口
- 统一调用所有房间的 `Update()` 方法

**关键变更**:
```csharp
// 之前
private readonly ConcurrentDictionary<string, RoomInfo> _rooms = new();

// 之后
private readonly ConcurrentDictionary<string, Room> _rooms = new();
```

### 3.2 Room 类实现

**核心方法**:

1. **构造函数**: 初始化房间信息、帧同步控制器、逻辑房间
2. **AddPlayer**: 添加玩家到房间
3. **RemovePlayer**: 从房间移除玩家
4. **StartGame**: 开始游戏，初始化帧同步
5. **EndGame**: 结束游戏，停止帧同步
6. **Update**: 更新房间状态（由 RoomManager 调用）
7. **HandleInput**: 处理玩家输入

**状态管理**:
- 房间状态通过 `RoomInfo.Status` 管理
- 帧同步状态通过 `FrameSyncController.IsActive` 管理
- 游戏逻辑状态通过 `LogicRoom` 管理

### 3.3 FrameSyncController 实现

**从 FrameSyncManager 提取的逻辑**:
- `RoomFrameSyncState` 的所有字段和方法
- 帧输入缓冲和收集逻辑
- 帧推进逻辑
- 帧数据发送逻辑

**保留在 FrameSyncManager 的逻辑**:
- 全局帧同步配置（帧率、间隔）
- 逻辑环境初始化（如果需要全局共享）

### 3.4 迁移策略

**阶段1: 创建新类**
- 创建 `Room` 类
- 创建 `FrameSyncController` 类
- 保持现有代码不变

**阶段2: 逐步迁移**
- 修改 `RoomManager` 管理 `Room` 实例
- 将 `FrameSyncManager` 的逻辑迁移到 `FrameSyncController`
- 更新 `GameServer` 调用新接口

**阶段3: 清理旧代码**
- 移除 `RoomFrameSyncState` 类
- 简化 `FrameSyncManager`（如果需要保留）
- 清理不再使用的代码

## 4. 关键决策与取舍

### 问题：FrameSyncManager 是否还需要保留？

**备选方案**:
1. **完全移除**: 所有逻辑都在 Room 中
2. **保留为协调器**: 管理全局配置和初始化
3. **保留为工具类**: 提供帧同步相关的工具方法

**选择**: 方案2 - 保留为协调器

**理由**:
- 逻辑环境初始化可能需要全局共享
- 帧同步配置可能需要统一管理
- 便于后续扩展（如房间间通信）

**影响**:
- FrameSyncManager 职责大幅简化
- 主要逻辑都在 Room 和 FrameSyncController 中

### 问题：Room 如何访问网络和用户管理？

**备选方案**:
1. **依赖注入**: 通过构造函数传入
2. **单例模式**: Room 直接访问单例
3. **服务定位器**: 通过服务定位器获取

**选择**: 方案1 - 依赖注入

**理由**:
- 便于测试（可以注入 Mock 对象）
- 依赖关系明确
- 符合依赖倒置原则

**影响**:
- RoomManager 需要传递依赖
- 构造函数参数较多

### 问题：LogicRoom 如何管理？

**备选方案**:
1. **Room 直接管理**: Room 创建和管理 LogicRoom
2. **延迟创建**: 游戏开始时才创建 LogicRoom
3. **外部管理**: 由 FrameSyncController 管理

**选择**: 方案2 - 延迟创建

**理由**:
- LogicRoom 只在游戏开始时需要
- 减少不必要的资源占用
- 符合按需创建原则

**影响**:
- StartGame 时需要创建 LogicRoom
- 需要处理创建失败的情况

## 5. 与现有系统的关系

### 5.1 与 GameServer 的关系

- GameServer 通过 RoomManager 管理房间
- GameServer 处理网络消息，调用 Room 的方法
- GameServer 统一调用 RoomManager.UpdateAllRooms()

### 5.2 与 UserManager 的关系

- Room 通过 UserManager 查询用户信息
- Room 通过 UserManager 获取 SessionId 发送消息
- UserManager 不直接管理房间，只管理用户

### 5.3 与 NetworkManager 的关系

- Room 通过 NetworkManager 发送消息给客户端
- NetworkManager 不关心房间逻辑，只负责消息传输
- Room 通过 UserManager 获取 SessionId，再通过 NetworkManager 发送

## 6. 测试策略

### 6.1 单元测试

- Room 的玩家管理功能
- FrameSyncController 的输入处理和帧推进
- RoomManager 的房间生命周期管理

### 6.2 集成测试

- 房间创建到游戏开始的完整流程
- 帧同步数据发送和接收
- 玩家加入和离开房间

### 6.3 性能测试

- 多房间并发更新性能
- 帧同步延迟测试
- 内存占用测试

---

**相关文档**:
- [Frame-Sync-Mechanism 帧同步机制](../05-CoreArchitecture%20核心架构/帧同步/Frame-Sync-Mechanism%20帧同步机制.md)
- [Network-Multiplayer 联机模式](../09-GameModes%20游戏模式/Network-Multiplayer%20联机模式.md)

---

*文档版本：v1.0*  
*创建时间：2025-01-27*  
*最后更新：2025-01-27*  
*状态：设计完成，待Review*  
*Owner*: AI Assistant  
*变更摘要*: 创建房间系统重构技术设计文档，提出面向对象的重构方案

