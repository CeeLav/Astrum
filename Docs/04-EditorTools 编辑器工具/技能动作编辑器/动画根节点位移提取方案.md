...
| 字段名 | 类型 | 说明 |
|--------|------|------|
| `root_motion_data` | string | 数组格式的根节点位移数据（可选，逗号分隔的整型） |
...
```csharp
// CSV 字段值（字符串格式，整型）
root_motion_data = "60,0,0,0,0,0,0,1000,50,0,0,0,0,0,1000,100,0,0,0,0,0,1000,80,0,0,0,0,0,1000,..."

// 解析后：
// frameCount = 60
// Frame 0: deltaPosition = (0, 0, 0), deltaRotation = (0, 0, 0, 1) [单位：米/弧度]
// Frame 1: deltaPosition = (0.05, 0, 0), deltaRotation = (0, 0, 0, 1)
// Frame 2: deltaPosition = (0.1, 0, 0), deltaRotation = (0, 0, 0, 1)
// Frame 3: deltaPosition = (0.08, 0, 0), deltaRotation = (0, 0, 0, 1)
// ...

// 运行时转换：
// int dx = 50; → FP deltaX = (FP)50 / 1000 = 0.05
```
...
namespace Astrum.Editor.RoleEditor.Services
{
    ...
    public static class AnimationRootMotionExtractor
    {
        ...
        public static string SerializeToArrayString(AnimationRootMotionData motionData)
        {
            ...
            // 格式: "frameCount,dx0,dy0,dz0,rx0,ry0,rz0,rw0,dx1,dy1,dz1,rx1,ry1,rz1,rw1,..."
            var parts = new List<string> { motionData.Frames.Count.ToString() };
            ...
            foreach (var frame in motionData.Frames)
            {
                ...
                parts.Add(((int)(frame.DeltaPosition.x * SCALE)).ToString());
                parts.Add(((int)(frame.DeltaPosition.y * SCALE)).ToString());
                parts.Add(((int)(frame.DeltaPosition.z * SCALE)).ToString());
                
                // DeltaRotation 四元数 (x, y, z, w) - 浮点数 * 1000 转为整型
                parts.Add(((int)(frame.DeltaRotation.x * SCALE)).ToString());
                parts.Add(((int)(frame.DeltaRotation.y * SCALE)).ToString());
                parts.Add(((int)(frame.DeltaRotation.z * SCALE)).ToString());
                parts.Add(((int)(frame.DeltaRotation.w * SCALE)).ToString());
            }
...
private void LoadAnimationForAction(SkillActionEditorData action)
{
    ...
    if (clip != null)
    {
        ...
        {
            // 将位移数据序列化为数组字符串（用于保存到CSV）
            action.RootMotionDataString = AnimationRootMotionExtractor.SerializeToArrayString(action.RootMotionData);
            
            Debug.Log($"[SkillActionEditor] Extracted root motion for action {action.ActionId}: " +
                      $"{action.RootMotionData.TotalFrames} frames, " +
                      $"data size: {action.RootMotionDataString.Length} bytes");
        }
        else
        {
            action.RootMotionDataString = string.Empty;
        }
    }
...
private void ApplyRootMotion(Entity entity, int actionId, int currentFrame)
{
    ...
    // 应用增量旋转（如果动画包含根节点旋转）
    // 大多数技能动作不需要旋转，但某些特殊动作（如翻滚、旋转攻击）可能需要
...
public ActionInfo GetAction(int actionId, int entityId)
{
    ...
    if (skillActionTable?.RootMotionData != null && 
        !string.IsNullOrEmpty(skillActionTable.RootMotionData))
    {
        try
        {
            // 直接从数组格式字符串转换为运行时数据（整型转定点数）
            // 这是最高效的方式，避免中间浮点数转换
            actionInfo.RootMotionData = AnimationRootMotionExtractor.ConvertToRuntimeFromArrayString(
