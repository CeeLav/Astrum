...
1. [系统概述](#系统概述)
2. [运行时架构](#运行时架构)
3. [SkillActionCapability - 技能动作执行能力](#skillactioncapability---技能动作执行能力)
...
触发层：TriggerFrame（触发帧事件）
   ↓
运行时层：SkillActionCapability（内部执行） → HitManager/SkillEffectManager → DamageCalculator
   ↓
...
```
技能运行时层
├── SkillActionCapability（技能动作执行能力）
...
| 模块 | 职责 | 输入 | 输出 |
|-----|------|------|------|
| SkillActionCapability | 技能执行调度 | SkillAction + TriggerFrame | 即时命中检测 / SkillEffect请求 |
...
SkillActionCapability 在实体上运行，负责：
...
```csharp
public class SkillActionCapability
{
...
{
    // 1. SkillActionCapability 处理触发帧（包含即时命中检测与入队效果）
    SkillActionCapability.ProcessFrame(owner, skillActionInfo, currentFrame);
...
public class HitManager
{
    ...
    /// 即时命中查询（无注册、无生命周期）
    /// </summary>
    public IEnumerable<Entity> QueryHits(Entity owner, HitBoxData boxData)
    {
        // 计算世界坐标下的攻击盒位置
        var worldCenter = owner.Transform.position + boxData.Offset;
        var worldRotation = owner.Transform.rotation;

        switch (boxData.ShapeType)
        {
...
### 4.3 HitBox 数据结构（推荐，对齐 BEPUPhysics v2）
...
public struct HitBoxData
{
    ...
    public Vector3 LocalOffset;         // 本地偏移（骨骼/角色局部）
    public Quaternion LocalRotation;    // 本地旋转（与造型方向对齐）

    // 几何参数
    public Vector3 Size;                // Box 半尺寸*2（长宽高）
    ...
    public Vector3 SweepDirection;      // Sweep/Raycast 方向（世界系）
    public float SweepDistance;         // Sweep/Raycast 距离

    // 过滤
    public int TargetLayerMask;         // 物理图层过滤（与 BEPU 碰撞组映射）
...
当前帧开始
    ↓
1. 所有 SkillActionCapability 更新
    ├─ 执行当前技能动作逻辑
    └─ 更新动作帧计数
    ↓
2. SkillActionCapability 处理触发帧
    ├─ 检查当前帧是否有触发事件
...
```
Frame 5:
    → SkillActionCapability 触发 Collision(4001)
...
Frame 15:
    → SkillActionCapability 触发 Collision(4002)
...
Frame 25:
    → SkillActionCapability 触发 Collision(4003)
...
采用即时检测策略：在触发帧由 `SkillActionCapability` 直接判定是否满足自定义条件（能量、状态标记、资源是否充足等），满足则立即触发效果。
...
GameLogicSystem (主循环)
    ↓
SkillActionCapability (技能能力)
    ↓
SkillActionCapability（内部执行）
    ↓
┌──────────────────┬──────────────────┐
...
| 现有系统 | 集成点 | 说明 |
|---------|-------|------|
| **Action系统** | SkillActionCapability | 在 SkillAction 的 Update 中调用 SkillActionCapability |
