...
3. **禁用 Tag 信息** - 查看哪些 Capability Tag 被禁用，以及是谁禁用的
4. **消息日志** - 查看其他模块发送给该实体的消息（逻辑层 EventQueue 和视图层 ViewEventQueue）
...
- **ADDED**: 禁用 Tag 展示 - 显示被禁用的 Capability Tag 及其禁用发起者
- **ADDED**: 消息日志区 - 实时显示发送给该实体的逻辑层消息（EntityEvent）和视图层消息（ViewEvent）
